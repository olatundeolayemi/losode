...
export default function KidsClothingPage() {
  ...
  const allProducts = clothingProducts.slice(0, 40);
  const designers = ['All', ...new Set(allProducts.map((p) => p.designer))];

  let filteredProducts = [...allProducts];
  
  if (selectedFilters.category !== 'All') {
    filteredProducts = filteredProducts.filter(p => p.subcategory === selectedFilters.category);
  }
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-[300px] md:h-[400px] w-full">
        <Image
          src="/images/kids-outfit-dress-1.jpg"
          alt="Kids' Clothing"
          fill
        ...
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
          <div className="text-center text-white">
            <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4">Kids' Clothing</h1>
            <p className="text-lg md:text-xl max-w-2xl mx-auto px-4">
              Colorful and comfortable clothing for children
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="sticky top-32 space-y-6">
              <div>
                ...
                <div className="space-y-2">
                  {['All', 'Dresses', 'Shirts', 'Tops', 'Jumpsuits', 'Sets'].map((cat) => (
