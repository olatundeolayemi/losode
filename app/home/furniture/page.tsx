...
import { homewareProducts } from '@/lib/products';

export default function FurniturePage() {
  const [sortBy, setSortBy] = useState('newest');
  const [showFilters, setShowFilters] = useState(false);

  const allProducts = homewareProducts.filter(
    p => p.category === 'Homeware' || p.category?.toLowerCase().includes('furniture')
  ).slice(0, 40);
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="relative h-[300px] md:h-[400px] w-full">
        <Image
          src="/images/product-furniture.jpg"
          alt="Furniture"
          fill
        ...
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
          <div className="text-center text-white">
            <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4">Furniture</h1>
            <p className="text-lg md:text-xl max-w-2xl mx-auto px-4">
              Premium furniture pieces for your home
            </p>
          </div>
        </div>
      </div>
